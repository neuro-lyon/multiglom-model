...
paramfile_path='runs/two_glom/'
nproc=12
db_filename="db_two_glom_beta_hom.h5"
...
if gen_param:
    ...
    d = {('Common', 'inter_conn_rate', '*', '*'): 
                {'range': linspace(start=0., stop=1., num=6),
                'unit': 1},
            ('Common', 'inter_conn_strength', '*', '*'): 
                {'range': linspace(start=0., stop=1., num=6),
                'unit': 1},
            #~ ('Common', 'N_subpop'): 
            #~ {'range': [1],
    ...
    gen_parameters('paramsets/std_beta.py', d, paramfile_path)
...
if plot_results:    
    ...
    list_plot=[0,20,38] # index of simulation to plot
